...
async function extractSiteData(url) {
	...
	const headStyleLinkElements = headElement.querySelectorAll('link[rel="stylesheet"]')
	const bodyStyleLinkElements = bodyElement.querySelectorAll('link[rel="stylesheet"]')

	//extract the script data from the head
	const headScriptsData = []
	for (let i = 0; i < headScriptElements.length; i++) {
		const scriptData = await extractScriptData(headScriptElements[i], `head-script${i}`)
	...
	for (let i = 0; i < bodyScriptElements.length; i++) {
		const scriptData = await extractScriptData(bodyScriptElements[i], `body-script${i}`)
	...
	for (let i = 0; i < headStyleLinkElements.length; i++) {
		const styleData = await extractStyleSheetData(headStyleLinkElements[i], `head-style${i}`)
	...
	for (let i = 0; i < bodyStyleLinkElements.length; i++) {
		const styleData = await extractStyleSheetData(bodyStyleLinkElements[i], `body-style${i}`)
...
function convertSiteDataToFileReadyContent(siteData) {
	...
	for (let i = 0; i < siteData.headStylesData.length; i++) {
		...
		parentNode.insertAdjacentHTML('beforeend', `<link rel="stylesheet" href=./${siteData.headStylesData[i].name}.css>`)
	...
	for (let i = 0; i < siteData.bodyStylesData.length; i++) {
		...
		parentNode.insertAdjacentHTML('beforeend', `<link rel="stylesheet" href=./${siteData.bodyStylesData[i].name}.css>`)
	...
	for (let i = 0; i < siteData.headScriptsData.length; i++) {
		...
		parentNode.insertAdjacentHTML('beforeend', `<script src=./${siteData.headScriptsData[i].name}.js>${"" /*siteData.headScriptsData[i].text*/}</script>`)
	...
	for (let i = 0; i < siteData.bodyScriptsData.length; i++) {
		...
		parentNode.insertAdjacentHTML('beforeend', `<script src=./${siteData.bodyScriptsData[i].name}.js>${"" /*siteData.bodyScriptsData[i].text*/}</script>`)
...
function writeFileReadyContent(fileReadyContent, folderPath) {
	...
	//write the html file
	fs.writeFileSync(path.join(folderPath, 'index.html'), fileReadyContent.html.content)

	//write the head scripts
	for (let i = 0; i < fileReadyContent.headScripts.length; i++) {
		fs.writeFileSync(path.join(folderPath, `${fileReadyContent.headScripts[i].name}.js`), fileReadyContent.headScripts[i].content)
	}

	//write the body scripts
	for (let i = 0; i < fileReadyContent.bodyScripts.length; i++) {
		fs.writeFileSync(path.join(folderPath, `${fileReadyContent.bodyScripts[i].name}.js`), fileReadyContent.bodyScripts[i].content)
	}

	//write the head styles
	for (let i = 0; i < fileReadyContent.headStyles.length; i++) {
		fs.writeFileSync(path.join(folderPath, `${fileReadyContent.headStyles[i].name}.css`), fileReadyContent.headStyles[i].content)
	}

	//write the body styles
	for (let i = 0; i < fileReadyContent.bodyStyles.length; i++) {
		fs.writeFileSync(path.join(folderPath, `${fileReadyContent.bodyStyles[i].name}.css`), fileReadyContent.bodyStyles[i].content)
	}
}
